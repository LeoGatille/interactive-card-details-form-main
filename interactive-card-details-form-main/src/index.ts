...
const context = {
    ...
    components: {
        form: {
            props: { //* At this point I'm commited to create a weak VueJs desing 
            ...
            init() {
                this.data.form.model = new Form(
                    this.props.reactiveFormKeys.map(key => createReactiveForm(key, true)),
                    'validateBtn',
            ...
            onValidate() {
                //! This is just bad
                if (this.form.model.isValid) return true;

                //! Bad sideEffect. Should be higher
                //? realTimeValidation could become a prop
                if (!this.form.model.realTimeValidation) this.form.model.realTimeValidation = true;
                return false
            },
            destroy() {
